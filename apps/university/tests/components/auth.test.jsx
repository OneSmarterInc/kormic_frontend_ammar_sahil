...
import Login from '../../src/pages/auth/UniversityLoginPage';
...
beforeEach(() => Object.assign(auth, { status: 'guest', user: { role: 'university', university_id: 7 }, loginWithPassword: vi.fn(), completeTotpLogin: vi.fn() }));
function login() {
...
test('password challenge, invalid TOTP, then successful verification', async () => {
  ...
  auth.completeTotpLogin.mockRejectedValueOnce(new Error('Invalid code')).mockResolvedValueOnce({ role: 'university', university_id: 7, name: 'Operator' });
...
function guard() {
  render(<MemoryRouter initialEntries={['/private']}><Routes><Route element={<RequireAuth />}><Route element={<RequireRole role="university" />}><Route path="/private" element={<p>Protected content</p>} /></Route></Route><Route path="*" element={<p>Access redirected</p>} /></Routes></MemoryRouter>);
}
