...
test('cached user data survives serialization', () => {
  const user = { id: 1, role: 'institute', totp_enrolled: true };
  setCachedUser(user);
...
test('portal landing route points to its dashboard', () => {
  assert.equal(roleHome(), '/institute/dashboard');
});
...
test('password and MFA requests stay bound to this portal', async () => {
  ...
  assert.deepEqual(requests, [
    { url: '/auth/web/login/', body: { email: 'officer@example.com', password: 'password', portal: 'institute' } },
    { url: '/auth/web/verify-totp/', body: { mfa_token: 'challenge', code: '123456', portal: 'institute' } },
  ]);
});
